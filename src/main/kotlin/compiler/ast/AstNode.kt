...
data class ACallExpr(val name: String, val args: List<AExpr>, val location: Location) : AstNode(location), ABlockCommand, AExpr
open class AParamDeclaration(val name: String, val location: Location) : AstNode(location), ADeclaration, AExprOrIdentifierDeclaration {
    override fun toString(): String {
        return "${this::class.java.simpleName}('$name')"
    }
}
data class AVarDeclaration(val name: String, val value: AExpr, val location: Location) : AstNode(location), ADeclaration {
    override fun toString(): String {
        return "${this::class.java.simpleName}('$name' = '$value')"
    }
}

data class AVarDeclarations(val declarations: List<AVarDeclaration>, val location: Location?) : AstNode(location), ABlockCommand

data class AIdentifier(val name: String, val location: Location) : AstNode(location), AAtomicExpr
data class ABinaryOp(val operator: BinaryOperator, val left: AExpr, val right: AExpr, val location: Location) : AstNode(location), AExpr
...
data class AAssignStmt(val left: AIdentifier, val right: AExpr, val location: Location) : AstNode(location), AStmtInNestedBlock, ABlockCommand
data class ABlock(val body: List<ABlockCommand>, val location: Location?) : AstNode(location), AStmt, ABlockCommand
data class AIfStmt(val guard: AExpr, val ifBranch: ABlock, val elseBranch: ABlock? = null, val location: Location) : AstNode(location), ABlockCommand, AStmtInNestedBlock
